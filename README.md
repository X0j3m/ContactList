...
| Method | Description |
| :--- | :--- |
| `Login()` | Returns generated JWT token. If token generation fails, returns `400 BadRequest`. |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| GET | http://localhost:8080/api/Login | Get authentication token | n/a | 200 OK (Authentication token with expiration time) / 400 BadRequest (Error generating JWT token) | No |
...
| POST | http://localhost:8080/api/Contacts | Create a new contact | JSON (Example below) | 201 Created (body: created id) / 400 BadRequest | Yes |
| PUT | http://localhost:8080/api/Contacts/{id} | Update contact by id | JSON (Example below) | 204 NoContent / 400 BadRequest / 404 NotFound | Yes |
| DELETE | http://localhost:8080/api/Contacts/{id} | Delete contact by id | n/a | 204 NoContent / 404 NotFound | Yes |
